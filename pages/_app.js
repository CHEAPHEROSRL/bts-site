...
function getColorScale(themePrimary) {
  if (!themePrimary) return null;
  const colorName = themePrimary.toLowerCase();
  return colorPalettes[colorName] || colorPalettes.grey;
}

function App({ Component, pageProps, mainNav, footerNav, globals, ctaButtons, newsletterForm }) {
  const themePrimary = globals?.theme?.primary;
  const colorScale = getColorScale(themePrimary);
