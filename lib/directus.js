...
export async function fetchGlobals() {
  try {
    // Fetch globals with nested social_links data
    const res = await fetch(`${DIRECTUS_URL}/items/globals?fields=*,social_links.*`);
    const json = await res.json();
    return json?.data || null;
  } catch (e) {
...
// Directus Flows API functions (requires DIRECTUS_ADMIN_TOKEN)
export async function createFlow(flowData) {
  const token = process.env.DIRECTUS_ADMIN_TOKEN;
  if (!DIRECTUS_URL || !token) return null;
  
  try {
    const res = await fetch(`${DIRECTUS_URL}/flows`, {
      method: 'POST',
      headers: {
...
export async function createOperation(operationData) {
  ...
  try {
    const res = await fetch(`${DIRECTUS_URL}/operations`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
...
export async function updateFlow(flowId, data) {
  const token = process.env.DIRECTUS_ADMIN_TOKEN;
  if (!DIRECTUS_URL || !token || !flowId) return null;
  
  try {
    const res = await fetch(`${DIRECTUS_URL}/flows/${flowId}`, {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
